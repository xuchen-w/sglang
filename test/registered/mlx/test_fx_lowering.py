...
def test_exported_executor_refreshes_replaced_attribute_without_recapture():
    from sglang.srt.hardware_backend.mlx.fx_lowering import _resolve_graph_attr
    ...
    with mock.patch(
        "sglang.srt.hardware_backend.mlx.fx_lowering._resolve_graph_attr",
        wraps=_resolve_graph_attr,
    ) as resolve:
        actual = executor(value)[0]
        resolve.assert_called_once_with(graph, "weight")
    torch.mps.synchronize()
